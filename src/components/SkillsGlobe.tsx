...
const SKILLS: Skill[] = [
  ...
  { name: 'Linux',         slug: 'linux',              color: '111111' },
  { name: 'Bash',          slug: 'gnubash',            color: '111111' },
...
export default function SkillsGlobe() {
  ...
  useEffect(() => {
    const update = () => {
      ...
      // Em telas pequenas usa quase toda largura disponível; cresce até 560 em desktop.
    ...
    const tick = () => {
      ...
      if (items) {
        ...
        items.forEach((el, i) => {
          ...
          // Billboard: contra-rotaciona ícone pra sempre encarar câmera
  ...
  return (
    <div className="relative w-full">
      ...
      >
        <div
          ...
          style={{
            ...
            background:
              'radial-gradient(circle at 35% 30%, rgba(255,255,255,0.5), rgba(0,0,0,0.04) 55%, rgba(0,0,0,0.10) 100%)',
            border: '1px solid rgba(0,0,0,0.08)',
            boxShadow:
              'inset 0 0 60px rgba(0,0,0,0.05), 0 30px 60px -30px rgba(0,0,0,0.25)',
          }}
        />
        <div
          aria-hidden
          ...
          style={{
            ...
            transform: 'translate(-50%, -50%)',
            background:
              'radial-gradient(circle at 50% 50%, rgba(0,0,0,0.05), rgba(0,0,0,0) 55%)',
          }}
        />
        ...
        >
          {/* faint guide rings (equador + meridiano) */}
                ...
                >
                  {isActive && (
                    <span
                      ...
                      style={{
                        background:
                          'radial-gradient(circle, rgba(220,60,40,0.35), rgba(220,60,40,0) 70%)',
                      }}
                    />
                  )}
                  <img
                    src={s.src ?? `https://cdn.simpleicons.org/${s.slug}/${s.color ?? '111111'}`}
                    alt=""
                    width={32}
                    height={32}
                    loading="lazy"
                    ...
                    style={{
                      pointerEvents: 'none',
                      filter: 'drop-shadow(0 4px 6px rgba(0,0,0,0.18)) drop-shadow(0 1px 2px rgba(0,0,0,0.12))',
                    }}
                  />
                </button>
              </div>
        ...
        <div className="absolute inset-x-0 bottom-2 flex justify-center pointer-events-none">
          <div className={`px-4 py-1.5 bg-paper border border-ink/15 font-mono text-[11px] uppercase tracking-[0.2em] text-ink transition-opacity ${label ? 'opacity-100' : 'opacity-0'}`}>
            {label ?? '—'}
          </div>
        </div>
      </div>
      ...
      <p className="mt-4 text-center font-mono text-[10px] uppercase tracking-[0.22em] text-muted">
        Arraste pra girar · clique pra fixar o nome
      </p>
    </div>
  )
}
